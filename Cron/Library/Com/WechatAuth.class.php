...
class WechatAuth {
    ...
    public function __construct($appid, $secret, $token = null){
        if($appid && $secret){
            ...
            customError('缺少参数 APP_ID 和 APP_SECRET .');
        }
    }
     ...
     */
    public function getAccessToken($type = 'client', $code = null,$no_cache=0){
        $param = array(
            'appid'  => $this->appId,
        ...
        switch ($type) {
            case 'client':
                ...
                $at = S($this->appId.'_client_accessToken');
                if($at){
        ...
        if(is_array($token)){
            if(isset($token['errcode'])){
                customError($token['errmsg']);
            } else {
                $this->accessToken = $token['access_token'];
                
                
                if($type=="client"){
                    S($this->appId.'_client_accessToken',$this->accessToken,7000);
                }
                            
                addWxLog($this->appId,$this->accessToken,'get_token');
                
                return $this->accessToken;
            }
        } else {
            customError('获取微信access_token失败！');
        }
    }
     ...
     */
    public function getUserInfo_new($openid, $lang = 'zh_CN'){
        if(!$this->accessToken){
            $this->getAccessToken('client',null,0);
        }
        $query = array(
        ...
        $info = self::http("https://api.weixin.qq.com/cgi-bin/user/info", $query);
    ...
    public function getUserInfo($openid, $lang = 'zh_CN'){
        if(!$this->accessToken){
            $this->getAccessToken('code',null,1);
        }
        $query = array(
            'access_token' => $this->accessToken,
            'openid'       => $openid,
    ...
    //发送模板消息   add  by  dddd
    function sendTemplateMessage($tplid,$touser,$content,$url='',$topcolor = "#FF0000",$access_token=''){
    
        if(!$content || !$tplid || !$touser ) return '';
        ...
        //print_test($access_token);
    
        $url = "https://api.weixin.qq.com/cgi-bin/message/template/send?access_token=$access_token";
        $res = curlPost($url,json_encode($data));
    
        //addWxLog($this->appId,"templateMessage\r\n".$res['errcode'].' '.$res['errmsg']);
    
        $log = array();
        $log ['cTime'] = time ();
        $log ['cTime_format'] = date ( 'Y-m-d H:i:s', $log ['cTime'] );
        $log ['content'] = is_array ( $data ) ? serialize ( $data ) : $data;
        $log ['token'] = $this->appId;
        $log ['touser'] = $data['touser'];
        $log ['code'] = $res['errcode'];
        $log ['errmsg'] = $res['errmsg'];
        $log ['tplid'] = $data['template_id'];
        M("wx_tpl_msg_log")->add($log);
    ...
    function OAuthWeixin($callback) {
        ...
        if (! isset ( $_GET ['state'] )) {
            ...
            if($content ['openid']){
                ...
                //$_SESSION['wx_openid'] = $content ['openid'];
                session("wx_openid",$content['openid']);
                //print_test($_SESSION);
                $t = explode("?", $callback);
                $callback = $t[0];
                //print_test($content,0);exit($callback);
                redirect ( $callback );
            }
            else {
                //print_test($content);
                customError("授权失败，请稍后再试！");
            }
