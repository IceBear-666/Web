...
class ResumeController extends UserController {
    ...
    public function testresume() {
       ...
       $sid  = I('get.sid',1); $this->assign('info', array('pid'=>0,"cid"=>0,"status"=>1));
        if($myinfo['type']==1){
    ...
    public function educationExperience() {
        ...
        if( !$data['education'] || !$data['endYear'] || !$data['professional'] || !$data['schoolName'] ){
    ...
    public function projectExperience() {
        ...
        if( !$data['startDate'] || !$data['endDate'] || !$data['projectRemark']
            || !$data['positionName'] || !$data['projectName'] ){
            $this->ResumeJson(0,"请填写完整！");
        }
        else{
            if(!$projectid){
