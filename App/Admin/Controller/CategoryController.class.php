...
class CategoryController extends AdminController {
    ...
    public function index(){
        $tree = D('Category')->getTree(0,'id,name,title,sort,pid,allow_publish,status,leixing,video_show');
    ...
    public function cache(){
        ...
        $data = $all = array();
        $list = M("category")->where(array('pid'=>0,'status'=>1))->order("sort asc,id asc")->select();
        foreach ($list as $k=>$v){
            
            $all[$v['id']] = $v;
            
            $sub = $zhiwei_leibie = array();
            $slist = M("category")->where(array('pid'=>$v['id'],'status'=>1))->order("sort asc,id asc")->select();
            foreach ($slist as $kk=>$vv){
                $temp = array(
                    ...
                    'name'=>$vv['name'],
                    'title'=>$vv['title'],
                    'create_time'=>date("Y-m-d H:i:s",$vv['create_time']),
                    'status'=>$vv['status'],
                    'video_show'=>$vv['video_show'],
                );
                if($vv['leixing']){
                    $zhiwei_leibie[$vv['id']] = $temp;
                }
                else $sub[$vv['id']] = $temp;
            ...
            $data[$v['id']] = array(
                ...
                'status'=>$v['status'],
                'sub'=>$sub,
                'zhiwei_leibie'=>$zhiwei_leibie,
                'video_show'=>$v['video_show'],
            );
        }
        
        //F("webCategory",$data);
        
        $filename       =   DATA_PATH . 'webCategory.php';
        $str='<?php return ' . var_export($data,true) . '; ?>';
        file_put_contents($filename, $str);
        
        
        $filename       =   DATA_PATH . 'webCategoryAll.php';
