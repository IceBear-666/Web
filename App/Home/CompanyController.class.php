...
class CompanyController extends UserController {
    ...
    public function companylist(){
        ...
        $info = M("Olapplication o")
            ...
            ->where($map)->field("o.*,c.title ctitle")->order('o.id asc')->select();
        $collet_list =  M("Keep k")
         ->join("  LEFT JOIN  Olapplication o on o.id = k.cid")
         ->where(array("k.uid"=>$uid ) )->select();
            
		//$collet_list =  M("Keep")->where(array("uid"=>$uid) )->order('id asc')->select();
    ...
    */
	public function keep() {		
		...
		if ($db->add($data)) {
			//收藏成功时对该内容的收藏数+1
			//M('Olapplication')->where(array('id' => $wid))->setInc('keep');
			echo 1;
		} else {
			echo 0;
		}
	...
	Public function cancelKeep () {
        ...
        if($_POST['wid']){
            ...
            if($wid){
                M("Keep")->where(array('kid'=>$kid))->delete();
                if ($wid) {
                    echo 1;
                }
