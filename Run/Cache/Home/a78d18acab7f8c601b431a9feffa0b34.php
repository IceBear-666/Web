...
<div class="w960 mt30">
   ...
   <div class="lw240">
     <div class="navbg">
      <h2 class="shownav">行业领域<i class="btnsj <?php if(!empty($bigid)){echo 'btnsjhv';}elseif(!empty($hy)){echo 'btnsjhv';} ?>"></i></h2>
         <?php geturl2('领域',$bigid,'bigid'); ?>
          <?php geturl2('行业',$hy,'hy'); ?>
        <div class="mod-menu  <?php if(!empty($hy)){echo 'dn';}elseif(!empty($bigid)){echo 'dn';} ?>">


          <!-- 一级分类 -->
          <ul class="menu-item">

            <?php if(is_array($hangye_lingyu)): $kk = 0; $__LIST__ = $hangye_lingyu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($kk % 2 );++$kk;?><li><a href="/Company/index/<?php echo ($ct); ?>-<?php echo ($xz); ?>-0-<?php echo ($order); ?>-<?php echo ($v["id"]); ?>?kd=<?php echo ($kd); ?>" ><?php echo ($v["title"]); ?></a></li>
         ...
         <div class="menu-cont">
         	...
         	<?php if(is_array($hangye_lingyu)): $kk = 0; $__LIST__ = $hangye_lingyu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($kk % 2 );++$kk;?><div class="menu-cont-list">

	               <?php if(is_array($v["sub"])): $mm = 0; $__LIST__ = $v["sub"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$m): $mod = ($mm % 2 );++$mm;?><a href="/Company/index/<?php echo ($ct); ?>-<?php echo ($xz); ?>-<?php echo ($m["id"]); ?>-<?php echo ($order); ?>-0?kd=<?php echo ($kd); ?>" <?php if($mm == $hy): ?>class="current"<?php endif; ?> data-ifs="<?php echo ($mm); ?>" ><?php echo ($m["title"]); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
        ...
        <div class="fgxian"></div>
         ...
         <h2 class="shownav">公司性质<i class="btnsj <?php if(!empty($xz))echo 'btnsjhv'; ?>"></i></h2>
          <?php geturl2('公司性质',$xz,'gs'); ?>

         <div class="mygsxz  <?php if(!empty($xz)){echo 'dn';} ?>">
             <?php if(is_array($gongsi_xingzhi)): $k = 0; $__LIST__ = $gongsi_xingzhi;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($k % 2 );++$k;?><a href="/Company/index/<?php echo ($ct); ?>-<?php echo ($k); ?>-<?php echo ($hy); ?>-<?php echo ($order); ?>-<?php echo ($bigid); ?>?kd=<?php echo ($kd); ?>" data-fs="<?php echo ($k); ?>" <?php if($k == $xz): ?>class="current"<?php endif; ?>><?php echo ($v); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
         </div>
  ...
  <!-- 右侧列表开始-->
   <div class="rw680">
     ...
     <div class="shaixuan">
       ...
       <a href="/Company/index/0-<?php echo ($xz); ?>-<?php echo ($hy); ?>-<?php echo ($order); ?>?kd=<?php echo ($kd); ?>" <?php if($ct == 0): ?>class="cur"<?php endif; ?>>不限</a><em>|</em>

       <?php $cnum = count($city_list);$ci=0;foreach($city_list as $k=>$v){ $ci++; ?>
       		<a href="/Company/index/<?php echo ($k); ?>-<?php echo ($xz); ?>-<?php echo ($hy); ?>-<?php echo ($order); ?>?kd=<?php echo ($kd); ?>" <?php if($ct == $k): ?>class="cur"<?php endif; ?> ><?php echo ($v["name"]); ?></a><?php if($cnum!=$ci) echo'<em>|</em>'; ?>
       <?php } ?>
       <div class="clr"></div>
     </div>
     ...
     <div class="shaixuan">
       <span>排序：</span>
       <a href="/Company/index/<?php echo ($ct); ?>-<?php echo ($xz); ?>-<?php echo ($hy); ?>-0?kd=<?php echo ($kd); ?>" <?php if($order == 0): ?>class="cur"<?php endif; ?> >人气</a><em>|</em>
       <a href="/Company/index/<?php echo ($ct); ?>-<?php echo ($xz); ?>-<?php echo ($hy); ?>-1?kd=<?php echo ($kd); ?>" <?php if($order == 1): ?>class="cur"<?php endif; ?>>最新更新</a>
       <div class="clr"></div>
     </div>
     ...
     <?php if(!$list) echo '<div class="nocenter"><span><img src="/Public/Home/images/empty.png" />未找到符合要求的记录</span></div>'; ?>
       <ul>
       	...
       	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><li <?php if($key%3== '0'): ?>class="clr"<?php endif; ?>>
               <h5>
                  <a class="comtl lf" href="/Company/info/id/<?php echo ($v["com_id"]); ?>" target="_blank"><?php echo ($v["company_short_name"]); ?></a>
                  <a class="rt" href="/Company/info/id/<?php echo ($v["com_id"]); ?>" target="_blank">[<?php echo ($v["company_city"]); ?>]</a>
                  <div class="clr"></div>
               </h5>
               <a  href="/Company/info/id/<?php echo ($v["com_id"]); ?>" target="_blank" class="comlogo">
               ...
               <div class="zhiwei">
                 <?php if(is_array($v["jobs_arr"])): $i = 0; $__LIST__ = $v["jobs_arr"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vv): $mod = ($i % 2 );++$i;?><a href="/Jobs/info/id/<?php echo ($vv["jid"]); ?>/source/company_list" target="_blank"><?php echo ($vv["zhiwei_mingcheng"]); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
               </div>
...
<?php if($page_count > 0): ?><script>
$(function(){
	/*分页 */
 	$('.Pagination').pager({
	      currPage: "<?php echo ($currPage); ?>",
	      pageNOName: "pn",
	      form: "searchForm",
	...
	/* $('.hc_list a.cl_job').click(function(e){
		e.preventDefault();
		var href=$(this).attr("href");
		window.location.href=href+"?source=company_list"; //公司列表页
	}); */
	//end
})
</script><?php endif; ?>
...
$(function(){
    $(".shownav").click(function(){
        ...
        $(this).next().slideToggle(200);//需要展开的内容
    });
