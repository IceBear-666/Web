        ...
        <div class="rw680">
            ...
            <div class="jm_companylist_r" style="margin:0;position: relative;">
                ...
                <table class="jm_companylist_tb">
                    <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; if($v["cid"] == 1): $now = strtotime(date('y-m-d',time())); $end_date = strtotime($v['end_time']);?>
                        <!-- $v['cid'] == "1" && --> 
                        <?php if ($now <= $end_date ) :?>
                            <tr>
							   ...
							   <!--  <td width="40"><div id="jobCollection" class="jd_col"><i class="icon-resumebold icon-favourite"></i><span class='keep' wid='<?php echo ($v["id"]); ?>'>收藏<?php if($v["keep"]): ?>(<?php echo ($v["keep"]); ?>)<?php endif; ?></span><i class="dn collection_jd_trangle"></i></div></td> -->
                            </tr>
