...
<body data-spy="scroll" data-target="#myScrollspy" data-offset="10" style="position: relative;">
    ...
    <div class="w960 jm_companylist_container">  
        ...
        <div class="rw680">
           ...
           <div id="section-6" class="jm_companylist_section jm-other">
                    <h3 class="jm_support_title jm_cl_darkgreen"><i class="icon-other-title"></i>其他</h3>
                    <table class="jm_companylist_tb">
                    <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; if($v["cid"] == 203): $now = strtotime(date('y-m-d',time())); $end_date = strtotime($v['end_time']);?>
                    ...
                    <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; if($v["cid"] == 84): $now = strtotime(date('y-m-d',time())); $end_date = strtotime($v['end_time']);?>
