<?php if (!defined('THINK_PATH')) exit(); if(is_array($tree)): $i = 0; $__LIST__ = $tree;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><dl class="cate-item">
		<dt class="cf" <?php if($list['leixing']) echo 'style="background-color: #efefef;"'; ?> >
			<form action="<?php echo U('edit');?>" method="post">
				...
				<div class="order" style="text-align:left;width:200px"><!-- <?php echo ($list['allow_publish']?'是':'否'); ?> --><?php echo ($list["name"]); ?></div>
				<div class="order" style="text-align:center;width:120px;"> <?php echo ($list['leixing']?'职位类型':'行业领域'); ?> </div>
				<div class="order" style="text-align:center;width:100px;"> <?php echo ($list['video_show']?'是':'否'); ?> </div>
			</form>
		</dt>
