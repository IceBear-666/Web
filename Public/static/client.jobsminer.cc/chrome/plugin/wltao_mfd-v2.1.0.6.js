...
_wlt_mfd_Obj_ = $.extend(window._wlt_mfd_Obj_, {
	...
	svrUrls: {
		...
		//白名单查询
		'whitelist': 'https://api.walatao.com/tp/?c=mfd&a=whitelist_new',
	...
	tpl: {
		...
		'login-tip': '<div id="mfd-login-widgets" style="padding: 15px; text-align: center; background: #18b4ed; position: absolute; left: -5px; top: 50px; width: 150px;z-index: 999999;"><a style="margin-right: 15px; color: #fff;" target="_blank" href="http://we.walatao.com/login/">登录</a><a style="color: #fff;" target="_blank" href="http://we.walatao.com/account/register/">注册</a><div class="arrow" style="width: 16px;height: 16px;background: #18b4ed;transform: rotate(-45deg);position: absolute;left: 70px;top: -8px;"></div></div>'
	},
	...
	isSupport: function(){
		var supportSites = {
			...
			'www.amazon.co.jp'	: 	['amazon_jp', 'jp']
		},
		host = location.host;
		this.websiteKey = supportSites[host][0];
		this.webCountry = supportSites[host][1];
	...
	init: function(){
		...
		var mfdRun = function(){
			...
			var initStatus = function(){
				...
				var req = function(packObj){
					...
					orderinfo = $(packObj.trackBtn).parents('.shipment').siblings('.order-info').eq(0);
				...
				_this.orderPack.pushCall(req, 'mfd');
				_this.orderPack.ajaxCall(undefined, 'mfd');
			}
			...
			var catchOrderInfo = function(obj){
				...
				for(var j=0; j<packNum; j++){
					...
					shipmentId 	=	shipmentId.length > 1 ? shipmentId : trackBtn.attr('href').split('shipmentId=');
		...
		var screenshotRun = function(){
			...
			var doUpload = function(){
				...
				//var goodsImg = _this.orderGoodsPack();
			...
			var getShipInfo = function(){
				...
				_this.orderPack.pushCall(upPackInfo, 'mfd');
				_this.orderPack.ajaxCall(undefined,'mfd');
			}
			...
			var getGoodsImg = function(){
				var packInfo 	= {};
				var upGoodsImg 	= function(imgObj){
					//console.log('----------wltao-orderAjaxShip--------', packObj);
					if( imgObj ){
						imgObj = imgObj;
						_this.post(_this.svrUrls['uppack'], {
							uid		: 	wltao_funs.getUserId(),
							orderid	: 	orderid,
							ord_goods_img: 	encodeURIComponent(wltao_funs.toJsonStr(imgObj))
						},function(rs){
							console.log(rs.msg);
						});
					}
				}
				_this.orderGoodsPack.pushCall(upGoodsImg, 'mfdgoods');
				_this.orderGoodsPack.ajaxCall(undefined,'mfdgoods');
			}
		...
		//添加密封代DOM入口
		setTimeout(function(){
 			KISSY.use("wltao-orderAjaxShip-v15", function(a,b){
				_this.orderPack = new b(_this.webCountry);
			});
			...
			KISSY.use("wltao-mfdAjaxShip-v15", function(a,b){
				_this.orderGoodsPack = new b(_this.webCountry);
			});
		}, 200);
	},
...
console.log('wltao-mfdAjaxShip-v15 js running...');
KISSY.add("wltao-mfdAjaxShip-v15", function(KS){
	var JDom = $;
	var goodsInfo_divs	=	JDom('.a-fixed-left-grid'),	//商品div
		goodsrCtn		=	goodsInfo_divs.size(),	//订单数量
		orderGoodsPack		=	{},
		ajaxCallbacks	=	{};
	...
	/* 获取页面的订单商品信息 */
	var getOrderGoodsPack = function(pageType){
		var shipments, shipment, shipLinkBtn, shipLink, isShipped, ordShipCtn, shipmentId,
			goods, transComp, goodsInfo_div,trackBtnId,
			i, j;
		//if( !JDom.isEmptyObject(orderPack) ) return ;
		if( 'detail'===pageType ){
			for(i=0; i<goodsrCtn; i++){
				goodsInfo_div 	= 	goodsInfo_divs.eq(i);
				goods 		= 	goodsInfo_div.find('.a-col-right .a-link-normal');
				var goodsLink = document.location.origin + goods.eq(0).attr('href');
				var goodsName = JDom.trim(goods.eq(0).html());
				orderGoodsPack[i] = 	{};
				orderGoodsPack[i] = {
					'goodsName'	:	goodsName,
					'goodsLink'	:	goodsLink
		...
		//console.log('----------wltao-orderAjaxShip--------', orderGoodsPack);
		return ;
	}
	...
	var ajaxPackDetail = function(callback, cbKey){
		if( JDom.isEmptyObject(orderGoodsPack) ) return false;
		
		var getImgFromHtml = function(cont){
		...
		var packinfo, idx;
		var _TO = {};
		var cb = function(idx, shipid){
			var id = idx+'-'+shipid;
			if( orderGoodsPack[idx][shipid].shipNum ) {
				clearTimeout(_TO[id]);
				//console.log('----wltao-orderAjaxShip-v15------done11-----'+cbKey+'---'+id, orderGoodsPack[idx][shipid]['trackBtnId']);
				callbacksApply(orderGoodsPack[idx][shipid], callback, cbKey);
			}
			else{
				//console.log('----wltao-orderAjaxShip-v15------done22-----'+cbKey+'---'+id, orderGoodsPack[idx][shipid]['trackBtnId']);
				//reqStats[idx][shipid] = orderGoodsPack[idx][shipid].shipNum
				_TO[id] = setTimeout(function(){cb(idx, shipid);}, 300);
			}
			return ;
		};
		for(idx in orderGoodsPack){
			packinfo = orderGoodsPack[idx];
			
			//console.log('----wltao-orderAjaxShip-v15---done----'+cbKey+'---', orderGoodsPack[idx][shipid].trackBtnId);
			var itemid = (packinfo.goodsLink + '/').match('/[0-9A-Z]{10}/');
			if( itemid ){
				itemid = itemid[0].replace(/\//g,'');
			}
			var country = 'US';
			if(document.location.host.indexOf('www.amazon.de') > -1){
				country = 'DE';
			}else if(document.location.host.indexOf('www.amazon.co.jp') > -1){
				country = 'JP';
			}
			
			KS.io.get('https://api.walatao.com/mfdGoodsImg.php?itemid=' + itemid + '&country=' + country,
			function(cont){
				var cont = JSON.parse(cont);
				//var tmp = getImgFromHtml(cont);
				//var _packobj = orderGoodsPack[tmp.orderId][tmp.shipmentId] = KS.merge(orderGoodsPack[tmp.orderId][tmp.shipmentId], tmp);
				//console.log('----------wltao-orderAjaxShip-v15------ajaxret-----cbKey----', cbKey);
				//console.log('----------wltao-orderAjaxShip-v15------ajaxret----'+cbKey+'--');
				callbacksApply(cont, callback, cbKey);
			});
		}
	}
	...
	//console.log('----------wltao-orderAjaxShip-v15------amazonKey---------', amazonKey);
		...
		JDom.isEmptyObject(orderGoodsPack)&&getOrderGoodsPack(pt);
	};
