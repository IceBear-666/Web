...
//	});
	...
	$('#cancelDetail').click(function(){
		...
		$('#hasLabels').append('<li class="link">+添加标签</li>');
	...
	if($("#mypdtag").val()=="1"){
		...
		$.ajax({
        	...
        	if(result.success){
				...
				$('#hasLabels').append('<li class="link clr">+添加标签</li>');
        		companyLabels = $('#hasLabels').html();
				$('#hasLabels2').html(companyLabels);
				$('#hasLabels2 .link').hide();
...
$('#Member').on('keyup','.memberForm .s_textarea',function(){
		...
		if (len > 500) {
			 ...
			 $(this).siblings(".word_count").find('span').text(500 - len);
		 }
});
		...
		if(confirm('确定要删除该创始人吗？')){
			...
			if(leaderId==""){
				if($("#Member .member_wrap").length<=1){
					$(".member_info2:nth-child(1)",memberWrap).removeClass("dn");
					$(".member_info:nth-child(2)",memberWrap).addClass("dn");
					$(".member_info:nth-child(2) input[type!='submit'], .member_info:nth-child(2) textarea",memberWrap).val("");
					$(".member_info:nth-child(2) img",memberWrap).attr("src","");
					$(".member_info:nth-child(2) .portraitShow",memberWrap).hide();
 	 ...
 	 */
		...
		$("#editDetailForm").validate({
	    	...
	    	messages: {
    	   	...
    	   	companyUrl: {
    	    	required: "请输入公司网址，如：www.jobsminer.cc",
    	    	checkUrl:"请输入有效的公司网址",
    	    	maxlength:"请输入120字符以内的网址"
    	   	}
	    	},
				...
				var positionType =  $('#select_category').val() != $('#positionType').attr("placeholder") ? $('#positionType').val() :'';
    	   		...
    	   		$.ajax({
	            	...
	            	if(result.success){
						...
						$('#didian').text(result.content.city);
						$('#guimu').text(result.content.companySize);
						$('#gongsixz').text(result.content.gongsi_xingzhi);
						$('#hangye').text(result.content.industryField);
						$('#zhuye').text(result.content.companyUrl);
						$('#mygourl').attr('href','http://'+companyUrl);
	            		if($('#addLabels').css("display") == 'none'){
	            			$('.editDetail').hide();
	            			$('#editCompanyDetail').show();
	            			$('.c_detail').removeClass('c_detail_bg');
