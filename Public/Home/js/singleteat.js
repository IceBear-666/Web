...
projectTitle.each(function() {
	...
	popQR(), a = $("#customTitleName"), "" == a.val() ? $("#customBlock .cust_title span").text("自定义标题") : $("#customBlock .cust_title span").text(a.val()), 9 == $(".md_flag").length ? $(".mr_module_add").addClass("dn") : $(".mr_module_add").removeClass("dn"), initTitlePosition(), initMceMr(), b = globals.initRatio, updateRatioRM(b, b), c = $(".mr_myresume_r .scroll_fix").offset(), $(window).scroll(function() {
		...
		submitHandler: function(a) {//b = $('input[name="shenfen"]', a).val(),
			var c = $('input[name="mr_year"]', a).val(),
				...
				nativeplace = $('input[name="nativeplace"]', a).val(),
				nation = $('input[name="nation"]', a).val(),
				politicalstatus = $('input[name="politicalstatus"]', a).val(),
				live_city = $('input[name="live_city"]', a).val(),
				live_address = $('input[name="live_address"]', a).val(),
				edu_type = $('input[name="edu_type"]', a).val(),
				graduate_time = $('input[name="graduate_time"]', a).val(),
				hukou_city = $('input[name="hukou_city"]', a).val(),
				hukou_city_now = $('input[name="hukou_city_now"]', a).val(),
				expect_city = $('input[name="expect_city"]', a).val(),
	...
	B.obj.delegate(".mr_head_r", "click", function() {
		...
		"" == d && (kb(), d = "upJobForm", f = !1, b = "", b = $("#job_update_hide").html(), b = b.replace(/mce_jason/g, "upjobContent"), b = b.replace(/tinymce_replace/g, "tinymce1"), $(this).parents(".mr_jobe_list").hide().after(b), $(this).parents(".mr_jobe_list").next().attr("id", "upJobForm"), c = $("#upJobForm"), $("#mr_expid", c).val($(this).data("expid")), $(".mr_del_ok", c).attr("data-id", $(this).data("expid")), oMoudle.workId = $(this).data("expid"), $("img.logo_u", c).prev().prev().addClass("up"), e = $(this).data("logo"), "" != e ? (e = ctx + "/" + e, $("img.logo_u", c).css({
			...
			width: 0,
			height: 0
		}).attr("src", e), $("input[name='companyLogoHide']", c).val("")), $('input[name="companyName"]', c).val($(this).data("comname")), $('input[name="positionName"]', c).val($(this).data("posname")), $('input[name="startTime"]', c).val($(this).data("startime")), $(".mr_sta_time .mr_btn", c).val($(this).data("startime")), $('input[name="endTime"]', c).val($(this).data("endtime")), $(".mr_end_time .mr_btn", c).val($(this).data("endtime")), $("#upjobContent", c).val($(this).data("content")),$('input[name="salary_month"]', c).val($(this).data("salarymonth")),$('input[name="work_city"]', c).val($(this).data("workcity")),$('input[name="company_property"]', c).val($(this).data("companyproperty")), $('input[name="company_size"]', c).val($(this).data("companysize")), $('input[name="company_cat"]', c).val($(this).data("companycat")), $('input[name="work_cat"]', c).val($(this).data("workcat")), $('input[name="department"]', c).val($(this).data("department")), $('input[name="work_city"]', c).val($(this).data("workcity")), $('#workContent', c).val($(this).data("workcontent")), $('input[name="reterence_name"]', c).val($(this).data("reterencename")), $('input[name="reterence_relation"]', c).val($(this).data("reterencerelation")), $('input[name="reterence_company"]', c).val($(this).data("reterencecompany")), $('input[name="reterence_position"]', c).val($(this).data("reterenceposition")), $('input[name="reterence_phone"]', c).val($(this).data("reterencephone")), $('input[name="reasons"]', c).val($(this).data("reasons")),   initMceMr1(), h(), N(), K($("#upJobForm")), I = new components.CalendarWrapper({
		...
		if (P.clearError(), T(), "添加" == $(this).children("em").text()) {
			...
			// "" == d && (b.not(":hidden") && b.hide(), 
				...
				d = "addEduForm", f = !1, $("#addEduForm").removeClass("dn"), kb()
		...
		var b, c;
		"" == d && (kb(), d = "upSchPraForm", f = !1, b = "", b = $("#schoolpra_update").html(), $("#addSchPraForm").addClass("dn"), $(this).parents(".mr_jobe_list").hide().after(b), Q(), R(), $(this).parents(".mr_jobe_list").next().attr("id", "upSchPraForm"), c = $("#upSchPraForm"), $("#schproId", c).val($(this).data("schproid")), $(".mr_del_ok", c).attr("data-id", $(this).data("schproid")), oMoudle.schproId = $(this).data("schproid"), $('input[name="praCompanyName"]', c).val($(this).data("pracomname")), $('textarea[name="practiceDes"]', c).val($(this).data("pracdes")), $('input[name="degree_text"]', c).val($(this).data("edu")), $('input[name="degree_val"]', c).val($(this).data("edu")), $('input[name="graduate_text"]', c).val($(this).data("date")), $('input[name="graduate"]', c).val($(this).data("date")),$('input[name="startYear"]', c).val($(this).data("startdate")),$('input[name="school_city"]', c).val($(this).data("schoolcity")), $('input[name="degree"]', c).val($(this).data("degree")), $('input[name="academy"]', c).val($(this).data("academy")) , $('input[name="major"]', c).val($(this).data("major")), $('input[name="gpa_score"]', c).val($(this).data("gpa")), $('input[name="professional_ranking"]', c).val($(this).data("prorank")),$('input[name="class_ranking"]', c).val($(this).data("classrank")), $('input[name="professional_describe"]', c).val($(this).data("prodes")), $('input[name="tutor_name"]', c).val($(this).data("tutorname")), $('input[name="tutor_phone"]', c).val($(this).data("tutorphone")),SPE())
		...
		SP.AddCancel()
			...
			submitHandler: function(a) {
				...
				$(a).find(":submit").val("保存中...").attr("disabled", !0), $.ajax({
					...
					b.success ? (c = b.content.isOpenMyResume, d = $(".toggle"), e = b.content.firstOpen, globals.isFirstOpen = e, c && ($(".openresume_tip").hide(), globals.isFirstOpen ? (globals.isOpenResume = "3", $(".firstset").show()) : globals.isOpenResume = d.hasClass("toggle-off") ? "0" : "1"), SPW(b), globals.token = b.resubmitToken, SPS(a), updateResumeTime(b.content.refreshTime),  g = parseInt($.trim($(".mr_bfb").text())), h = amountScore - g + f, updateRatioRM(f, h), SP.obj.find(".mr_head_r").trigger("click"), i = b.content.educationExperiences, SPV(a), j = SP.obj.find(".mr_empty_add"), j.not(":hidden") && j.hide(), k = $(".mr_module .flag_edu"), k.is(":hidden") || k.addClass("dn")) : alert(b.msg), $(a).find(":submit").val("保 存").attr("disabled", !1)
				})
			}
		}),SP = {
		obj: $("#schoolPractice"),
		...
		var b, c, e, g;
		"" == d && (kb(), d = "upProForm", f = !1, b = "", b = $("#pro_update_hide").html(), b = b.replace(/mce_jason/g, "upproContent"), b = b.replace(/tinymce_replace/g, "tinymce1"), $(this).parents(".mr_jobe_list").hide().after(b), $(this).parents(".mr_jobe_list").next().attr("id", "upProForm"), c = $("#upProForm"), e = $(this).parents(".mr_jobe_list"), initMceMr1(), h(), db(e.attr("data-id")), c.find(".mr_del_ok").data("id", e.data("id")), $('input[name="reterenceName"]', c).val($(this).data("retername")),$('#projectDutyedit', c).val($(this).data("produty")),$('input[name="projectNumber"]', c).val($(this).data("pronum")),$('input[name="reterencePhone"]', c).val($(this).data("reterphone")),c.find('input[name="projectName"]').val($.trim(e.find(".projectTitle").text())), c.find('input[name="thePost"]').val($(this).data("posiname")), g = $.trim(e.find(".mr_content_r span").text()).split(" - "), c.find('input[name="startTime"]').val(g[0]).prev().val(g[0]), c.find('input[name="endTime"]').val(g[1]).prev().val(g[1]), c.find('textarea[name="upproContent"]').val($.trim(e.find(".mr_content_m").html())), c.find('input[name="pro_link"]').val($.trim(e.find(".projectTitle").attr("href"))), K($("#upProForm")), Y.checkState(), ab = new components.CalendarWrapper({
			container: c.find(".mr_timed_div:first"),
			onchange: function(a, b) {
