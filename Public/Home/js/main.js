...
function placeholderFn() {
	if (!placeholderSupport()) $("[placeholder]").focus(function() {
		var e = $(this);
		if (e.val() == e.attr("placeholder")) e.val(""), e.removeClass("placeholder")
	}).blur(function() {
		var e = $(this);
...
var cloud_time = 2e3,
	jsoncallback = function(e) {
		alert("调用"), alert(e), console.log(e)
	};
$(function() {
	function e() {
		function e() {
	...
	$("#email,#password").focus(function() {
		...
		messages: {
			email: {
				required: "请输入登录邮箱地址",
				email: "请输入有效的邮箱地址，如：event@eztojob.com",
		...
		submitHandler: function(e) {
			...
			$(e).find(":submit").attr("disabled", !0), $.ajax({
				...
				if (1 == t.status) {
					var n = "/User/index.html";
					window.location.href = n
				} else if (240 == t.status) $("#beError").text("请输入常用邮箱地址").show();
				else if (210 == t.status) $("#beError").text("请输入100字以内的邮箱地址").show();
				else if (220 == t.status) $("#beError").text("请输入有效的邮箱地址，如：event@eztojob.com").show();
				else if (241 == t.status) $("#beError").text("请输入密码").show();
				else if (211 == t.status) $("#beError").text("请输入6-16位密码").show();
				else if (400 == t.status) $("#beError").text("该帐号不存在或密码错误，请重新输入").show();
				else if (401 == t.status) $("#beError").text("登录失败，该帐号已被禁用").show();
				else if (500 == t.status) $("#beError").text("登录失败,系统内部异常").show();
				else if (299 == t.status) $("#beError").text("你的操作太过频繁，请稍后再试").show();
				else $("#beError").text(t.info).show();
	...
	$(".register_radio li input").click(function() {
		...
		submitHandler: function(e) {
			...
			$(e).find(":submit").attr("disabled", !0), $.ajax({
				...
				else if (220 == t.state) $("#beError").text("请输入有效的邮箱地址，如：event@eztojob.com").show();
				else if (241 == t.state) $("#beError").text("请输入密码").show();
				else if (211 == t.state) $("#beError").text("请输入6-16位密码").show();
				else if (243 == t.state) $("#beError").text("请选择使用找份工作的目的").show();
