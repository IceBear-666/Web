...
projectTitle.each(function() {
	...
	function AllNum(a,b){
		...
		var num ="";
		num  += '<div class="progress-bar" style="width:'+ b/a*175 +'px"></div><span class="counter" style="left:70px">' + b +'/'+ a +'</span>' ;
		PR.obj.find("#progress-allresume").html(num)
	}
	function LowuserNum(a,b){
		var lownum ="";
		lownum  += '<div class="progress-bar" style="width:'+ b/a*65 +'px"></div><span class="counter" style="left:12px;">' + b +'/'+ a +'</span>' ;
				...
				$(b).find(":submit").val("保存中...").attr("disabled", !0), myresumeCommon.utils.addHttpPrefix("http://|https://", j, function(a) {
					...
					a.success ? (d = "", f = !0, mb(), $("#upProForm").remove(), b = a && a.content && a.content.projectExperiences || [], c = eb(b), Y.obj.find(".list_show").html(c), e = $(".mr_jobe_list .l2 .projectTitle"), e.each(function() {
						var a = $(this),
							b = a.width();
						a.find("span").css("left", b + 5)
					}), Y.UpCancel(), g = Y.obj.find(".mr_empty_add"), b.length ? g.hide() : g.show(), updateResumeTime(a.content.refreshTime), "" != oMoudleScore.projectExpScore && (amountScore -= oMoudleScore.projectExpScore), oMoudleScore.projectExpScore = "", h = a.content.infoCompleteStatus.score, i = parseInt($.trim($(".mr_bfb").text())), j = amountScore - i + h, updateRatioRM(h, j), Y.checkState(),tn = a.content.totalnum, fn = a.content.firstnum, NumPro(fn,tn),allnum = a.content.allnum, allemptynum = a.content.allemptynum, AllNum(allnum,allemptynum),lownum = a.content.lownum,lowemptynum = a.content.lowemptynum,LowuserNum(lownum,lowemptynum)) : alert(a.msg)
				})
			}
		})
	}
	...
	popQR(), a = $("#customTitleName"), "" == a.val() ? $("#customBlock .cust_title span").text("自定义标题") : $("#customBlock .cust_title span").text(a.val()), 9 == $(".md_flag").length ? $(".mr_module_add").addClass("dn") : $(".mr_module_add").removeClass("dn"), initTitlePosition(), initMceMr(), b = globals.initRatio, updateRatioRM(b, b), c = $(".mr_myresume_r .scroll_fix").offset(), $(window).scroll(function() {
		...
		C && C.hide(), E && E.hide(), D && D.hide(), Z && Z.hide(), _ && _.hide(), ab && ab.hide(), 
		bb && bb.hide(), G && G.hide(), H && H.hide(), I && I.hide(), J && J.hide(), Z && Z.hide(), _ && _.hide(), 
		b = $(".mr_self_state .mr_self_s"), 
		b.hasClass("active") || b.addClass("active"), 
		c = $(".set_default_wrap"), c.hasClass("active") && c.trigger("click")
	}), $(".ul_xl").delegate("li", "click", function() {
		$("#edu").val($(this).text()), $(this).parent().parent().hide(), $(".select_color").removeClass("select_color")
	...
	$("#olinfoForm").validate({
		...
		submitHandler: function(a) {//b = $('input[name="shenfen"]', a).val(),
			var gr = globals.resumeId,
				...
				profile =$('input[name="profile"]', a).val(),
			...
			$(a).find(":submit").val("保存中...").attr("disabled", !0), $.ajax({	
				...
				i.success ? (mb(),
					...
					NumInfo(fn,tn),
					allnum = i.content.allnum, 
					allemptynum = i.content.allemptynum, 
					AllNum(allnum,allemptynum),
					lownum = i.content.lownum,
					lowemptynum = i.content.lowemptynum,
					LowuserNum(lownum,lowemptynum),
		...
		var b = globals.resumeId,
				...
				n.success ? (mb(), d = "", f = !0, updateResumeTime(n.content.refreshTime), 
					...
					NumOtherInfo(fn,tn),
					allnum = n.content.allnum, 
					allemptynum = n.content.allemptynum, 
					AllNum(allnum,allemptynum),
					lownum = n.content.lownum,
					lowemptynum = n.content.lowemptynum,
					LowuserNum(lownum,lowemptynum),
					//$("#olinfoForm .mr_info_edit").addClass("dn"), 
	...
	, $(".mr_created").on("click", ".mr_md_del", function(a) {
		...
		$(this).text("删除中...").attr("disabled", !0), h = $(this), $.ajax({
			...
			if (c.success) {
				...
				case "upTrainingForm":
					$("#upTrainingForm .mr_cancel").trigger("click"),console.log(c),
					...
					break;
				case "upCertiForm":
					$("#upCertiForm .mr_cancel").trigger("click"),console.log(c),
					...
					break;
				case "upSchAwForm":
					$("#upSchAwForm .mr_cancel").trigger("click"),console.log(c),
					...
					break;
				case "upSchPraForm":
					$("#upSchPraForm .mr_cancel").trigger("click"),console.log(b),
					...
					break;
				case "upSchClubForm":
					$("#upSchClubForm .mr_cancel").trigger("click"),console.log(c),
		...
		PX.AddCancel()
			...
			submitHandler: function(a) {
				...
				$(a).find(":submit").val("保存中...").attr("disabled", !0), $.ajax({
					...
					var c, d, e, f, g, h, i, j,cc,dd,ee,ff,hh,ii,jj, k, l , m, n, o, p, r;
					if(b.success){
						$("#show_no_edu").hide();
					}
					b.success ? (c = b.content.isOpenMyResume, 
