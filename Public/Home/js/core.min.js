...
function errorTips(a, b) {
	$.colorbox({
		html: '<div id="uploadFile" class="popup"><table width="100%"><tr><td align="center"><h4 class="error_msg">' + a + "</h4></td>" + "</tr>" + "<tr>" + '<td align="center"><a href="javascript:;" class="btn_s">确&nbsp;定</a></td>' + "</tr>" + "</table>" + "</div>",
		title: b ? b : "错误提示"
	})
}
...
var cloud_time, feedback, closeBtn, showBtn, fdForm, ensureBtn, error, config;
$(function() {
	$("#email,#password").blur(function() {
		...
		$(this).parent().hasClass("slideUp") ? ($(this).children("em").removeClass("transform"), $(this).parent().children("dd").slideDown(200), $(this).parent().removeClass("slideUp").children("dd").slideDown(200)) : 
		($(this).children("em").addClass("transform"), $(this).parent().children("dd").slideUp(200), $(this).parent().addClass("slideUp").children("dd").slideUp(200))
		...
		$(this).prev(".license_upload").children("div").css("backgroundColor", "#ac6198")
	}), $(".business_license").on("mouseleave", 'input[type="file"]', function() {
		$(this).prev(".license_upload").children("div").css("backgroundColor", "#C3A8BC")
...
}(jQuery), feedback = $("#feedback-con"), closeBtn = $(".pfb-close"), showBtn = $("#feedback-iconnew"), fdForm = $("#product-fb"), ensureBtn = $(".ensure"), error = $("#feedback-iconnew .error"), config = {
	hide: function(a) {
		a.fadeOut()
	},
	show: function(a) {
	...
	valid: function(a) {
		...
		j = e.length, l = !0, ("" == c || d > 200) && (i.removeClass("dn").find("span").text("你还没填任何反馈呢"), k.addClass("txt"), b.setCountdown(3, i), l = !1), "" != e && (h = g.test(e)), h || "" == e || (i.removeClass("dn").find("span").text("请输入有效的邮箱地址"), k.hasClass("txt") && k.removeClass("txt"), b.setCountdown(3, i), l = !1), h && j > 100 && (i.removeClass("dn").find("span").text("请输入100字以内的邮箱地址"), k.hasClass("txt") && k.removeClass("txt"), b.setCountdown(3, i), l = !1), l && $.ajax({
			...
			a.success ? (config.hide(feedback), $("#product-fknew .fk-suc").removeClass("dn"), b.setCountdown(3, $("#product-fknew .fk-suc"))) : alert(a.msg)
		})
	}
}, showBtn.click(function() {
	config.show(feedback) //阿丁，这个代码是我做前台显示的，正常代码用下面注释的，好像需要返回值验证挺麻烦的，如果麻烦咱们可以不用，忽略此步直接提交。
	
	/*
	fdForm.find("input").val("").end().find("textarea").val(""), $.ajax({
		url: "http://service.lagou.com/feedback/check.json",
		type: "POST",
		data: {
			loginEmail: global.email,
			userId: global.usertoken
		...
		var b = $("#feedback-icon .fk-limit");
		a.success ? config.show(feedback) : (b.removeClass("dn"), config.setCountdown(3, b))
	})
	*/
